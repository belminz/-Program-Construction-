...
class Period {
    ...
    private String begin; //from
    private String ende; //to

    public  Period (String begin, String ende){
        this.begin=begin;
        this.ende=ende;
    }
      public String  from (){
...
public class Aufgabe1 {
    ...
    Map<String,Period> map = new TreeMap<>();


    public String from (String tätigkeit){
        if (map.containsKey(tätigkeit)){
            return map.get(tätigkeit).from();
        }
        else {
            return null;
        }
    }
    public String to (String tätigkeit){
        if (map.containsKey(tätigkeit)){
            return map.get(tätigkeit).to();
        }
        else {
            return null;
        }
    }
    public boolean add (String tätigkeit, Period period){
        if (!map.containsKey(tätigkeit)){
            map.put(tätigkeit,period);
            return false;
        }
        else {
            map.replace(tätigkeit,period);
            return true;
        }
...
public static void main(String[] args) {
    ...
    Period zeitraum1 = new Period("1.95.0", "2.5.0");
    plan1.add("Sonnenaufgang betrachten",zeitraum1);
    System.out.println(zeitraum1.from() +" "+ zeitraum1.to());
    //System.out.println(plan1.from("Sonnenaufgang betrachten") + " " + plan1.to("Sonnenaufgang betrachten"));

    Period zeitraum2 = new Period("0.0.2","0.0.3");
    plan2.add("Sonnenhut tragen", zeitraum2);
